...
`;try{const{stdout:n}=await Mn("powershell.exe",["-NoLogo","-NoProfile","-NonInteractive","-ExecutionPolicy","Bypass","-Command",e],{encoding:"utf8",maxBuffer:4194304}),t=n.trim();return ye(t)?t:null}catch{return null}}function On(e,n){const t=u.basename(e).toLowerCase();return n.some(r=>r.toLowerCase()===t)}function ye(e){try{return f.statSync(e).isFile()}catch{return!1}}async function Un(){if(j.platform()!=="win32")return[];const e=[];for(const n of Ne){const t=await Ft(n.id);t&&e.push({appId:n.id,name:n.name,path:t})}return e}const wt=ot.promisify(Le.execFile);async function zt(e){const n=W(e);if(!n)return!1;const t=[...new Set([n.processName,...n.exeNames].filter(Boolean))];if(j.platform()==="win32"){for(const r of t)try{const{stdout:a}=await wt("tasklist.exe",["/FI",`IMAGENAME eq ${r}`,"/FO","CSV","/NH"],{encoding:"utf8",windowsHide:!0});if(a.split(/\r?\n/).some(o=>o.trim().toLowerCase().startsWith(`"${r.toLowerCase()}"`)))return!0}catch{}return!1}for(const r of t)try{return await wt("pgrep",["-f",r],{encoding:"utf8"}),!0}catch{}return!1}async function qt(e,n){if(!/^[a-z0-9-]+$/i.test(e||""))return{success:!1,error:`Invalid appId: ${e}`};if(n!=null&&!/^[a-z0-9._-]+$/i.test(n))return{success:!1,error:`Invalid themeId: ${n}`};const t=W(e);if(!t)return{success:!1,error:`Unknown app: ${e}`};const r=t.defaultPort,a=[`--remote-debugging-port=${r}`];e==="codex"&&a.push("--disable-extensions"),n&&a.push(`--dream-theme=${n}`);try{const o=await zn(e);if(!o||!f.existsSync(o)||!f.statSync(o).isFile())return{success:!1,error:`Executable not found: ${o}`};if(j.platform()==="win32"&&!/\.exe$/i.test(o))return{success:!1,error:`Refusing to launch non-executable path: ${o}`};if(console.log(`[launcher] Killing existing ${e} instances...`),await Hn(e),await Fn(r,15e3),t.devToolsActivePort)try{f.unlinkSync(t.devToolsActivePort)}catch{}console.log(`[launcher] Launching ${o} with args: ${a.join(" ")}`);const s=Le.spawn(o,a,{detached:!0,stdio:"ignore",env:Bn()});s.unref(),console.log(`[launcher] Spawned process with PID: ${s.pid}`),console.log(`[launcher] Waiting for CDP port ${r} to be ready...`);let c=r;return t.devToolsActivePort?c=await Dn(t.devToolsActivePort,t.rendererHints,3e4):await jn(r,3e4),console.log(`[launcher] CDP port ${c} is ready`),e==="hana-agent"&&await Nn(c,t.rendererHints,3e4),{success:!0,port:c}}catch(o){return console.error("[launcher] Launch failed:",o),{success:!1,error:o.message}}}function Bn(){const e={...process.env};for(const n of["VITE_DEV_SERVER_URL","ELECTRON_RENDERER_URL","MAIN_VITE_DEV_SERVER_URL","ELECTRON_RUN_AS_NODE"])delete e[n];return e}async function Dn(e,n,t){const r=Date.now();let a=0;for(;Date.now()-r<t;){try{const o=f.readFileSync(e,"utf8").split(/\r?\n/,1)[0],s=Number(o);if(Number.isInteger(s)&&s>0)return a=s,await Ln(s,n,3e3),s}catch{}await new Promise(o=>setTimeout(o,500))}throw new Error(`DevToolsActivePort did not expose a live renderer${a?` on port ${a}`:""}: ${e}`)}async function Ln(e,n,t){const r=Date.now();for(;Date.now()-r<t;){try{const a=await fetch(`http://127.0.0.1:${e}/json/list`,{signal:AbortSignal.timeout(1e3)});if(a.ok){const o=await a.json();if(Array.isArray(o)&&o.some(s=>(s==null?void 0:s.type)==="page"&&n.some(c=>String(s.url).includes(c))))return}}catch{}await new Promise(a=>setTimeout(a,250))}throw new Error(`CDP renderer endpoint is not ready on port ${e}`)}async function Nn(e,n,t){const r=Date.now();let a="",o=0;for(;Date.now()-r<t;){try{const i=(await(await fetch(`http://127.0.0.1:${e}/json/list`,{signal:AbortSignal.timeout(1e3)})).json()).find(l=>(l==null?void 0:l.type)==="page"&&n.some(d=>String(l.url).includes(d)));if(i!=null&&i.id){if(i.id!==a)a=i.id,o=Date.now();else if(Date.now()-o>=3e3){console.log(`[launcher] Stable HanaAgent renderer ${a} confirmed`);return}}}catch{}await new Promise(s=>setTimeout(s,250))}throw new Error(`HanaAgent renderer did not stabilize on port ${e}`)}async function jn(e,n){const t=Date.now();let r="unknown";for(;Date.now()-t<n;)try{await new Promise((a,o)=>{const s=jt.createConnection(e,"127.0.0.1",()=>{s.end(),a()});s.once("error",c=>{r=c.message,o(c)}),setTimeout(()=>{s.destroy(),o(new Error("timeout"))},1e3)}),console.log(`[launcher] Port ${e} is open, verifying CDP endpoint...`),await Wn(e,15e3),console.log(`[launcher] CDP endpoint verified on port ${e}`);return}catch(a){r=a.message,console.log(`[launcher] Port check failed: ${a.message}, retrying...`),await new Promise(o=>setTimeout(o,1e3))}throw new Error(`CDP port ${e} did not become ready within ${n}ms (last error: ${r})`)}async function Wn(e,n){const t=Date.now();for(;Date.now()-t<n;)try{await new Promise((r,a)=>{const o=Nt.request({hostname:"127.0.0.1",port:e,path:"/json/version",method:"GET",timeout:2e3},s=>{let c="";s.on("data",i=>{c+=i}),s.on("end",()=>{s.statusCode===200?(console.log(`[launcher] CDP version response: ${c.substring(0,200)}`),r()):a(new Error(`HTTP ${s.statusCode}`))})});o.on("error",a),o.on("timeout",()=>{o.destroy(),a(new Error("timeout"))}),o.end()});return}catch(r){if(Date.now()-t>=n)throw r;await new Promise(a=>setTimeout(a,1e3))}}async function Hn(e){const n=j.platform(),t=W(e);if(!t)return;const r=[...new Set([t.processName,...t.exeNames].filter(Boolean))];try{if(n==="win32"){const{spawnSync:a}=require("child_process");for(const o of r)try{a("taskkill",["/T","/F","/IM",o],{stdio:"ignore"}),console.log(`[launcher] Killed existing ${o} process tree`)}catch{}}else if(n==="darwin"){const{spawnSync:a}=require("child_process");for(const o of r)try{a("pkill",["-f",o],{stdio:"ignore"}),console.log(`[launcher] Killed existing ${o} processes`)}catch{}}else if(n==="linux"){const{spawnSync:a}=require("child_process");for(const o of r)try{a("pkill",["-f",o],{stdio:"ignore"}),console.log(`[launcher] Killed existing ${o} processes`)}catch{}}}catch(a){console.warn("[launcher] Failed to kill existing instances:",a)}}async function Fn(e,n){const t=Date.now();for(;Date.now()-t<n;){if(!await new Promise(a=>{const o=jt.createConnection(e,"127.0.0.1");o.once("connect",()=>{o.destroy(),a(!0)}),o.once("error",()=>a(!1)),o.setTimeout(500,()=>{o.destroy(),a(!1)})})){console.log(`[launcher] Previous CDP port ${e} is closed`);return}await new Promise(a=>setTimeout(a,250))}throw new Error(`Existing ${e} CDP service did not stop; refusing to inject into the old application instance`)}async function zn(e){if(!W(e))throw new Error(`Unknown app: ${e}`);const t=j.platform();if(t==="win32"){const r=await Ft(e);if(r)return r}else if(t==="darwin"){const r=["/Applications/WorkBuddy.app","/Applications/ChatGPT.app"];for(const a of r)if(f.existsSync(a))return a}else if(t==="linux"){const r=e==="workbuddy"?["workbuddy","WorkBuddy"]:["codex","Codex"],a=["/usr/bin","/usr/local/bin","/opt",u.join(j.homedir(),".local","bin"),"/snap/bin"];for(const o of a)if(f.existsSync(o))for(const s of r){const c=u.join(o,s);if(f.existsSync(c))return c}for(const o of r)try{const{spawnSync:s}=require("child_process"),c=s("which",[o],{encoding:"utf8"}),i=String(c.stdout||"").trim();if(i&&f.existsSync(i))return i}catch{}}throw new Error(`Could not find ${e} executable`)}const qn=5e3,Vn=100,Gn=15e3,Kn=1e4,Jn=5e3;function Xn(e){if(!Number.isInteger(e)||e<1024||e>65535)throw new TypeError("port must be an integer from 1024 through 65535");return e}function ie(e,n,t={}){const r=t.allowZero?0:Number.EPSILON;if(!Number.isFinite(e)||e<r){const a=t.allowZero?"non-negative":"positive";throw new TypeError(`${n} must be a finite ${a} number`)}return e}function Vt(e){if(typeof e!="string"||e.length===0||e!==e.trim())throw new TypeError("webSocketDebuggerUrl must be a non-empty URL string");let n;try{n=new URL(e)}catch(t){throw new TypeError(`webSocketDebuggerUrl is invalid: ${t.message}`)}if(n.protocol!=="ws:"||n.hostname!=="127.0.0.1"||n.username||n.password||n.hash||!n.port)throw new TypeError("webSocketDebuggerUrl must use ws://127.0.0.1 with an explicit port");return Xn(Number(n.port)),n}function Zn(e,n){if(e===null||typeof e!="object"||Array.isArray(e)||e.type!=="page"||typeof e.url!="string"||typeof e.webSocketDebuggerUrl!="string")return!1;try{Vt(e.webSocketDebuggerUrl)}catch{return!1}return e.url.includes(n)}function We(e){if(e===null||typeof e!="object"||Array.isArray(e)||e.type!=="page"||typeof e.url!="string"||typeof e.webSocketDebuggerUrl!="string")return!1;try{return Vt(e.webSocketDebuggerUrl),!0}catch{return!1}}function Yn(e){return new Promise(n=>setTimeout(n,e))}async function yt(e,n){const t=Math.max(0,n.deadline-Date.now());let r=null;try{return await Promise.race([e,new Promise((a,o)=>{r=setTimeout(()=>{var s;(s=n.onTimeout)==null||s.call(n),o(new Error(`${n.label} timed out after ${n.timeoutMs}ms`))},t)})])}finally{r&&clearTimeout(r)}}async function pe(e,n,t={}){const r=ie(t.timeoutMs??Jn,"timeoutMs",{allowZero:!1}),a=t.fetchImpl??globalThis.fetch;if(typeof a!="function")throw new TypeError("fetchImpl must be a function");const o=`http://127.0.0.1:${e}/json/list`,s=new AbortController,c=Date.now()+r,i=t.quiet===!0;i||console.log(`[cdp] fetchRendererTargets: port=${e}, timeoutMs=${r}, endpoint=${o}`);let l;try{l=await yt(Promise.resolve(a(o,{redirect:"error",signal:s.signal})),{deadline:c,timeoutMs:r,label:"renderer target discovery",onTimeout:()=>s.abort()})}catch(m){throw i||console.log("[cdp] fetchRendererTargets error:",m),new Error(`failed to fetch renderer targets from ${o}: ${m.message}`)}if(l===null||typeof l!="object"||!l.ok)throw new Error(`renderer target discovery failed with HTTP ${(l==null?void 0:l.status)??"unknown"}`);let d;try{d=await yt(Promise.resolve(l.json()),{deadline:c,timeoutMs:r,label:"renderer target discovery JSON",onTimeout:()=>s.abort()})}catch(m){throw new Error(`malformed renderer target JSON from ${o}: ${m.message}`)}if(!Array.isArray(d))throw new Error("malformed renderer target JSON: expected an array");return d.filter(m=>Zn(m,n)).sort(er)}async function Qn(e,n,t={}){const r=ie(t.timeoutMs??qn,"timeoutMs",{allowZero:!0}),a=ie(t.pollMs??Vn,"pollMs",{allowZero:!1}),o=t.fetchImpl??globalThis.fetch;let s=0;const c=Date.now()+r;let i=new Error("no renderer discovery attempt completed");for(console.log(`[cdp] waitForRendererTargets: port=${e}, hint=${n}, timeoutMs=${r}`);;){try{const d=Math.max(1,Math.min(r-s,c-Date.now()));console.log(`[cdp] Attempting fetch: elapsed=${s}ms, remainingBudget=${d}ms, deadline=${c}`);const m=await pe(e,n,{fetchImpl:o,timeoutMs:d});if(m.length>0)return m;i=new Error("no matching renderer/index.html page targets")}catch(d){i=d instanceof Error?d:new Error(String(d)),console.log("[cdp] Fetch error:",i.message)}if(s>=r||Date.now()>=c)throw new Error(`timed out after ${r}ms waiting for renderer targets on 127.0.0.1:${e}: ${i.message}`);const l=Math.min(a,r-s);await Yn(l),s+=l}}class X{constructor(n,t={}){D(this,"webSocketDebuggerUrl");D(this,"WebSocketImpl");D(this,"commandTimeoutMs");D(this,"connectTimeoutMs");D(this,"socket",null);D(this,"nextRequestId",1);D(this,"pending",new Map);D(this,"socketOpen",!1);D(this,"opened",!1);D(this,"closed",!1);D(this,"closeStarted",!1);D(this,"terminalError",null);D(this,"openPromise",null);D(this,"resolveOpen",null);D(this,"rejectOpen",null);D(this,"connectTimer",null);this.webSocketDebuggerUrl=n;let r=null,a=null;try{r=require("ws")??null,r||(a="ws loaded but WebSocket is undefined")}catch(o){a=`ws require failed: ${(o==null?void 0:o.message)??o}`}if(!r)try{const o=require("undici");r=(o==null?void 0:o.WebSocket)??null,r||(a="undici loaded but WebSocket is undefined")}catch(o){a=`undici require failed: ${(o==null?void 0:o.message)??o}`}if(!r&&typeof globalThis.WebSocket=="function"&&(r=globalThis.WebSocket,a=null),!r){const o=a?` (${a})`:"";throw new Error(`No WebSocket implementation available for CDP${o}`)}this.WebSocketImpl=t.WebSocketImpl??r,this.commandTimeoutMs=ie(t.commandTimeoutMs??Gn,"commandTimeoutMs"),this.connectTimeoutMs=ie(t.connectTimeoutMs??Kn,"connectTimeoutMs")}open(){if(this.closed)return Promise.reject(this.terminalError??new Error("CDP session is closed"));if(this.opened)return Promise.resolve(this);if(this.openPromise)return this.openPromise;this.openPromise=new Promise((t,r)=>{this.resolveOpen=t,this.rejectOpen=r}),this.connectTimer=setTimeout(()=>{this.terminate(new Error(`CDP WebSocket connect timed out after ${this.connectTimeoutMs}ms`)),this.closeSocket()},this.connectTimeoutMs);try{this.socket=new this.WebSocketImpl(this.webSocketDebuggerUrl)}catch(t){return this.terminate(new Error(`failed to open CDP WebSocket: ${t.message}`)),this.openPromise}const n=this.socket;return n.onopen=()=>{this.closed||this.socketOpen||(this.clearConnectTimer(),this.socketOpen=!0,Promise.all([this.send("Runtime.enable"),this.send("Page.enable")]).then(()=>{if(this.closed)return;this.opened=!0;const t=this.resolveOpen;this.resolveOpen=null,this.rejectOpen=null,t==null||t(this)}).catch(t=>{this.terminate(t),this.closeSocket()}))},n.onmessage=t=>this.handleMessage(t),n.onerror=t=>{const r=t.error,a=r instanceof Error?r.message:typeof t.message=="string"&&t.message.length>0?t.message:"unknown socket error";this.terminate(new Error(`CDP WebSocket error: ${a}`)),this.closeSocket()},n.onclose=()=>{this.closeStarted=!0,this.terminate(new Error("CDP WebSocket closed"))},this.openPromise}send(n,t={},r={}){if(this.closed)return Promise.reject(this.terminalError??new Error("CDP session is closed"));if(!this.socketOpen||!this.socket)return Promise.reject(new Error("CDP session is not open"));if(typeof n!="string"||n.length===0)return Promise.reject(new TypeError("CDP method must be a non-empty string"));const a=ie(r.timeoutMs??this.commandTimeoutMs,"timeoutMs"),o=this.nextRequestId++;return new Promise((s,c)=>{const i=setTimeout(()=>{this.pending.delete(o),c(new Error(`CDP ${n} timed out after ${a}ms`))},a);this.pending.set(o,{resolve:s,reject:c,timer:i});try{this.socket.send(JSON.stringify({id:o,method:n,params:t}))}catch(l){clearTimeout(i),this.pending.delete(o),c(new Error(`failed to send CDP ${n}: ${l.message}`))}})}async evaluate(n,t={}){var a,o,s;if(typeof n!="string")throw new TypeError("Runtime.evaluate expression must be a string");const r=await this.send("Runtime.evaluate",{expression:n,awaitPromise:!0,returnByValue:!0},t);if(r!=null&&r.exceptionDetails)throw new Error(`Runtime.evaluate failed: ${((a=r.exceptionDetails.exception)==null?void 0:a.description)??r.exceptionDetails.text??"unknown JavaScript exception"}`);if(((o=r==null?void 0:r.result)==null?void 0:o.type)!=="undefined")return(s=r==null?void 0:r.result)==null?void 0:s.value}async addScriptToEvaluateOnNewDocument(n){const t=await this.send("Page.addScriptToEvaluateOnNewDocument",{source:n});return t==null?void 0:t.identifier}async removeScriptToEvaluateOnNewDocument(n){await this.send("Page.removeScriptToEvaluateOnNewDocument",{identifier:n})}close(){this.closeStarted||(this.terminate(new Error("CDP session closed by client")),this.closeSocket())}handleMessage(n){if(typeof n.data!="string"){this.terminate(new Error("received a non-text CDP WebSocket message")),this.closeSocket();return}let t;try{t=JSON.parse(n.data)}catch(a){this.terminate(new Error(`received malformed CDP JSON: ${a.message}`)),this.closeSocket();return}if(!Number.isInteger(t==null?void 0:t.id))return;const r=this.pending.get(t.id);if(r){if(this.pending.delete(t.id),clearTimeout(r.timer),t.error){r.reject(new Error(`CDP error: ${t.error.message}`));return}r.resolve(t.result)}}terminate(n){if(this.terminalError)return;this.clearConnectTimer(),this.terminalError=n,this.closed=!0,this.socketOpen=!1;const t=this.rejectOpen;this.resolveOpen=null,this.rejectOpen=null,t==null||t(n);for(const{reject:r,timer:a}of this.pending.values())clearTimeout(a),r(n);this.pending.clear()}clearConnectTimer(){this.connectTimer!==null&&(clearTimeout(this.connectTimer),this.connectTimer=null)}closeSocket(){if(this.closeStarted||(this.closeStarted=!0,!this.socket||typeof this.socket.close!="function"))return;const n=this.WebSocketImpl.CLOSING??2,t=this.WebSocketImpl.CLOSED??3;this.socket.readyState===n||this.socket.readyState===t||this.socket.close()}}function er(e,n){const t=[String(e.id??""),e.url,e.webSocketDebuggerUrl],r=[String(n.id??""),n.url,n.webSocketDebuggerUrl];for(let a=0;a<t.length;a++){if(t[a]<r[a])return-1;if(t[a]>r[a])return 1}return 0}function tr(){return u.join(P.app.getAppPath(),"themes")}function Gt(){const e=u.join(P.app.getPath("userData"),"themes");return f.mkdirSync(e,{recursive:!0}),e}function nr(){return[Gt(),tr()]}const vt=new Map;function He(e){var a;const n=[],t=new Set;for(const o of nr()){if(!f.existsSync(o))continue;const s=f.readdirSync(o,{withFileTypes:!0});for(const c of s){if(!c.isDirectory())continue;const i=u.join(o,c.name),l=u.join(i,"theme.json");if(f.existsSync(l))try{const d=JSON.parse(f.readFileSync(l,"utf-8")),m=cr(d);if(t.has(m.id))continue;const h=u.join(i,m.hero);if(!f.existsSync(h)||!f.statSync(h).isFile())throw new Error(`theme hero is missing: ${m.hero}`);if(e&&((a=m.apps[e])==null?void 0:a.compat)!==!0&&e!=="hana-agent")continue;t.add(m.id),n.push({id:m.id,name:m.name,author:m.author,path:i,manifest:m})}catch(d){console.error(`Failed to load theme ${c.name}:`,d)}}}const r=new Map;for(const o of n){const s=u.join(o.path,o.manifest.hero),c=tt(s),i=`${o.name.trim().toLocaleLowerCase()}\0${o.author.trim().toLocaleLowerCase()}\0${c}`,l=r.get(i);(!l||rr(o.id,l.id))&&r.set(i,o)}return[...r.values()].sort((o,s)=>o.name.localeCompare(s.name))}function tt(e){const n=f.statSync(e),t=vt.get(e);if(t&&t.size===n.size&&t.mtimeMs===n.mtimeMs)return t.hash;const r=st.createHash("sha256").update(f.readFileSync(e)).digest("hex");return vt.set(e,{size:n.size,mtimeMs:n.mtimeMs,hash:r}),r}function rr(e,n){const t=e.startsWith("custom-"),r=n.startsWith("custom-");return t!==r?!t:e.length<n.length||e.length===n.length&&e.localeCompare(n)<0}function Kt(e,n){return He(n).find(t=>t.id===e)}function ar(e){const n=Kt(e);if(!n)return;const t=u.resolve(n.path,n.manifest.hero);if(t.startsWith(`${u.resolve(n.path)}${u.sep}`))return t}function or(e){return`theme-asset://local/${encodeURIComponent(e)}`}function sr(e){const n=u.resolve(e.path),t=u.resolve(n,e.manifest.hero);if(t!==n&&!t.startsWith(n+u.sep))throw new Error(`Theme hero path escapes theme directory: ${e.manifest.hero}`);const r=f.readFileSync(t);return`data:${lr(e.manifest.hero)};base64,${r.toString("base64")}`}function ir(e,n,t){const r=tt(t);return He().some(a=>a.name.trim().toLowerCase()!==e.trim().toLowerCase()||a.author.trim().toLowerCase()!==n.trim().toLowerCase()?!1:tt(u.join(a.path,a.manifest.hero))===r)}function cr(e){if(typeof e!="object"||e===null||Array.isArray(e))throw new Error("theme manifest must be an object");if(e.schemaVersion!==1)throw new Error(`unsupported theme schema ${e.schemaVersion}`);if(typeof e.id!="string"||!/^[a-z0-9-]+$/.test(e.id))throw new Error("theme id must use lowercase letters, numbers, and hyphens");if(typeof e.name!="string"||!e.name.trim())throw new Error("theme name must be a non-empty string");if(typeof e.author!="string")throw new Error("theme author must be a string");if(typeof e.hero!="string")throw new Error("theme hero must be a string");let n;if(typeof e.video=="string"&&e.video.trim()){const r=e.video.trim();u.basename(r)!==r||!/\.(mp4|webm)$/i.test(r)?console.warn(`theme ${e.id}: ignoring invalid video field ${r}`):n=r}if(typeof e.colors!="object"||e.colors===null)throw new Error("theme colors must be an object");const t=["accent","secondary","surface","text"];for(const r of t)if(typeof e.colors[r]!="string"||!/^#[0-9a-fA-F]{6}$/.test(e.colors[r]))throw new Error(`theme color ${r} must be a hex color`);return{schemaVersion:1,id:e.id,name:e.name.trim(),author:e.author,hero:e.hero,video:n,colors:{accent:e.colors.accent,secondary:e.colors.secondary,surface:e.colors.surface,text:e.colors.text},copy:e.copy??void 0,apps:e.apps??{}}}function lr(e){const n=u.extname(e).toLowerCase();return{".png":"image/png",".jpg":"image/jpeg",".jpeg":"image/jpeg",".webp":"image/webp",".gif":"image/gif",".mp4":"video/mp4",".webm":"video/webm"}[n]||"image/png"}function dr(e){const n=e.manifest.video;if(!n)return null;const t=u.resolve(e.path);let r=u.resolve(t,n);if(r!==t&&!r.startsWith(t+u.sep)||!/\.(mp4|webm)$/i.test(u.extname(r)))return null;const a=P.app.getAppPath();if(a.endsWith(".asar")&&(t===a||t.startsWith(a+u.sep))){const o=u.resolve(a+".unpacked",u.relative(a,t));r=u.resolve(o,n)}try{if(!f.existsSync(r)||!f.statSync(r).isFile())return null}catch{return null}return r}const Jt=5,mr=32*1024*1024;let Ce=null;function ct(){try{const e=JSON.parse(f.readFileSync(Xt(),"utf8"));return lt(e)}catch{return[]}}function ur(e){const n=lt(e),t=[...ct()];for(const a of n){const o=t.findIndex(s=>s.id===a.id);o>=0?t[o]=a:t.push(a)}const r=t.slice(0,Jt);return Yt(r),r}function pr(e,n,t,r=4){const a=Qt()[e]??{};return[...n].sort((o,s)=>{if(o===t)return-1;if(s===t)return 1;const c=a[o]??{count:0,lastUsedAt:0},i=a[s]??{count:0,lastUsedAt:0};return i.lastUsedAt-c.lastUsedAt||i.count-c.count}).slice(0,r)}function nt(e,n){if(!/^[a-z0-9-]+$/i.test(e)||!/^[a-z0-9-]+$/i.test(n))return;const t=Qt(),r=t[e]??{},a=r[n]??{count:0};r[n]={count:a.count+1,lastUsedAt:Date.now()},t[e]=r,en(Zt(),t)}function hr(){return Ce||(Ce=new Promise((e,n)=>{const t=st.randomBytes(24).toString("hex"),r=Nt.createServer((a,o)=>{if(o.setHeader("Access-Control-Allow-Origin","*"),o.setHeader("Access-Control-Allow-Headers","Authorization, Content-Type"),o.setHeader("Access-Control-Allow-Methods","GET, PUT, POST, OPTIONS"),o.setHeader("Access-Control-Allow-Private-Network","true"),a.method==="OPTIONS"){o.writeHead(204).end();return}if(a.headers.authorization!==`Bearer ${t}`){o.writeHead(401).end("Unauthorized");return}if(a.url==="/theme-usage"&&a.method==="POST"){kt(a,o,s=>{if(typeof(s==null?void 0:s.appId)!="string"||typeof(s==null?void 0:s.themeId)!="string")throw new Error("Invalid theme usage payload");nt(s.appId,s.themeId),Je(o,200,{success:!0})});return}if(a.url!=="/custom-themes"){o.writeHead(404).end("Not found");return}if(a.method==="GET"){Je(o,200,ct());return}if(a.method!=="PUT"){o.writeHead(405).end("Method not allowed");return}kt(a,o,s=>{const c=lt(s);Yt(c),Je(o,200,c)})});r.once("error",n),r.listen(0,"127.0.0.1",()=>{const a=r.address();if(!a||typeof a=="string"){r.close(),n(new Error("Shared custom theme service did not expose a TCP port"));return}const o=`http://127.0.0.1:${a.port}`;e({endpoint:`${o}/custom-themes`,usageEndpoint:`${o}/theme-usage`,token:t})})}),Ce)}function Xt(){return u.join(P.app.getPath("userData"),"custom-themes.json")}function Zt(){return u.join(P.app.getPath("userData"),"theme-usage.json")}function Yt(e){en(Xt(),e)}function Qt(){try{const e=JSON.parse(f.readFileSync(Zt(),"utf8"));return e&&typeof e=="object"&&!Array.isArray(e)?e:{}}catch{return{}}}function en(e,n){f.mkdirSync(u.dirname(e),{recursive:!0}),f.writeFileSync(e,`${JSON.stringify(n,null,2)}
`)}function kt(e,n,t){let r=0;const a=[];e.on("data",o=>{if(r+=o.length,r>mr){n.writeHead(413).end("Payload too large"),e.destroy();return}a.push(o)}),e.on("end",()=>{if(!n.headersSent)try{t(JSON.parse(Buffer.concat(a).toString("utf8")))}catch(o){n.writeHead(400).end(o.message)}})}function lt(e){if(!Array.isArray(e))throw new Error("Custom themes must be an array");return e.slice(0,Jt).map((n,t)=>{var o,s;if(!n||typeof n!="object")throw new Error(`Invalid custom theme at index ${t}`);const r=n;if(typeof r.id!="string"||!/^custom-[a-z0-9-]+$/i.test(r.id))throw new Error(`Invalid custom theme id at index ${t}`);if(typeof r.name!="string"||!r.name.trim())throw new Error(`Invalid custom theme name at index ${t}`);if(typeof r.dataUrl!="string"||!/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(r.dataUrl))throw new Error(`Invalid custom theme image at index ${t}`);for(const c of["accent","secondary","surface","text"])if(typeof((o=r.colors)==null?void 0:o[c])!="string"||!/^#[0-9a-fA-F]{6}$/.test(r.colors[c]))throw new Error(`Invalid custom theme color ${c} at index ${t}`);const a=typeof((s=r.colors)==null?void 0:s.average)=="string"&&/^#[0-9a-fA-F]{6}$/.test(r.colors.average)?r.colors.average:void 0;return{id:r.id,name:r.name.trim(),dataUrl:r.dataUrl,colors:{accent:r.colors.accent,secondary:r.colors.secondary,surface:r.colors.surface,text:r.colors.text,...a?{average:a}:{}}}})}function Je(e,n,t){e.writeHead(n,{"Content-Type":"application/json; charset=utf-8"}),e.end(JSON.stringify(t))}function se(e){const n=/^#([0-9a-f]{6})$/i.exec(e);if(!n)return[255,255,255];const t=parseInt(n[1],16);return[t>>16&255,t>>8&255,t&255]}function Y(e){return"#"+e.map(n=>Math.max(0,Math.min(255,Math.round(n))).toString(16).padStart(2,"0")).join("")}function Xe(e){const n=e/255;return n<=.04045?n/12.92:Math.pow((n+.055)/1.055,2.4)}function de(e){return .2126*Xe(e[0])+.7152*Xe(e[1])+.0722*Xe(e[2])}function re(e,n){const t=de(e),r=de(n);return(Math.max(t,r)+.05)/(Math.min(t,r)+.05)}function Ae(e,n,t){return[0,1,2].map(r=>e[r]+(n[r]-e[r])*t)}function J(e){const n=e[0]/255,t=e[1]/255,r=e[2]/255,a=Math.max(n,t,r),o=Math.min(n,t,r),s=(a+o)/2;if(a===o)return[0,0,s];const c=a-o,i=s>.5?c/(2-a-o):c/(a+o);let l;return a===n?l=(t-r)/c+(t<r?6:0):a===t?l=(r-n)/c+2:l=(n-t)/c+4,[l/6,i,s]}function me(e,n,t){if(n<=0){const s=t*255;return[s,s,s]}const r=(s,c,i)=>(i<0&&(i+=1),i>1&&(i-=1),i<1/6?s+(c-s)*6*i:i<1/2?c:i<2/3?s+(c-s)*(2/3-i)*6:s),a=t<.5?t*(1+n):t+n-t*n,o=2*t-a;return[r(o,a,e+1/3)*255,r(o,a,e)*255,r(o,a,e-1/3)*255]}function gr(e,n,t){if(re(e,n)>=t)return e;const r=t+Math.max(.02,t*.02),a=de(e),o=de(n),s=a<o||a===o&&o>.475,[c,i,l]=J(e);if(i>=.02){let g=s?0:l,b=s?l:1;for(let C=0;C<14;C++){const A=(g+b)/2;re(me(c,i,A),n)>=r?s?g=A:b=A:s?b=A:g=A}const x=me(c,i,s?g:b);if(re(x,n)>=t)return x}const d=s?[0,0,0]:[255,255,255];let m=0,h=1;for(let g=0;g<12;g++){const b=(m+h)/2;re(Ae(e,d,b),n)>=r?h=b:m=b}return Ae(e,d,h)}function St(e,n,t){if(n.length===0)return e;const r=d=>Math.min(...n.map(m=>re(d,m)));let a=e;for(let d=0;d<4;d++){if(r(a)>=t)return a;const m=r(a);let h=n[0],g=1/0;for(const x of n){const C=re(a,x);C<g&&(g=C,h=x)}const b=gr(a,h,t);if(r(b)<=m+1e-9)break;a=b}if(r(a)>=t)return a;const[o,s]=J(e),c=[e,a];for(const d of[.02,.06,.12,.22,.78,.88,.95,.99])c.push(me(o,s,d));let i=a,l=r(a);for(const d of c){const m=r(d);m>l+1e-9&&(l=m,i=d)}return i}function fr(e){if(e.length<16||e.readUInt32BE(0)!==2303741511)return null;let n=8,t=-1;const r=[];for(;n+12<=e.length;){const o=e.readUInt32BE(n),s=e.toString("ascii",n+4,n+8),c=e.subarray(n+8,n+8+o);if(s==="IHDR"){const i=c.readUInt32BE(0),l=c.readUInt32BE(4),d=c[8],m=c[12];if(i!==1||l!==1||d!==8||m!==0)return null;t=c[9]}else if(s==="IDAT")r.push(c);else if(s==="IEND")break;n+=12+o}if(t<0||r.length===0)return null;let a;try{a=Cn.inflateSync(Buffer.concat(r))}catch{return null}if(a.length<2||a[0]>4)return null;switch(t){case 6:return a.length>=5?[a[1],a[2],a[3]]:null;case 2:return a.length>=4?[a[1],a[2],a[3]]:null;case 4:return a.length>=3?[a[1],a[1],a[1]]:null;case 0:return a.length>=2?[a[1],a[1],a[1]]:null;default:return null}}function br(){return String.raw`(function () {
...
`}const xr=30*60*1e3,wr=30*1e3,yr=128e3,Te=96;function Re(){return u.join(j.homedir(),".zcode","cli","db")}function vr(){const e=Re();let n=0;for(const t of["db.sqlite","db.sqlite-wal","db.sqlite-shm"])try{n=Math.max(n,f.statSync(u.join(e,t)).mtimeMs)}catch{}return n}function Oe(e,n){let t=0;for(let a=0;a<8;a++){const o=e[n+a];if(o===void 0)throw new Error("varint out of range");if(t=t*128+(o&127),!(o&128))return{value:t,next:n+a+1}}const r=e[n+8];if(r===void 0)throw new Error("varint out of range");return t=t*256+r,{value:t,next:n+9}}function kr(e,n,t){let r=0;for(let a=0;a<t;a++)r=r*256+e[n+a];return t<8&&r>=Math.pow(2,t*8-1)&&(r-=Math.pow(2,t*8)),r}function Ct(e){try{const n=Oe(e,0),t=n.value;if(t>e.length)return null;let r=n.next,a=t;const o=[];for(;r<t;){const s=Oe(e,r);r=s.next;const c=s.value;if(c===0){o.push(null);continue}if(c>=1&&c<=6){const i=[0,1,2,3,4,6,8][c];o.push(kr(e,a,i)),a+=i;continue}if(c===7){o.push(e.readDoubleBE(a)),a+=8;continue}if(c===8){o.push(0);continue}if(c===9){o.push(1);continue}if(c>=12&&c%2===0){const i=(c-12)/2;o.push(null),a+=i;continue}if(c>=13){const i=(c-13)/2;o.push(e.toString("utf8",a,a+i)),a+=i;continue}return null}return o}catch{return null}}function Sr(e){const n=e.indexOf("(");if(n<0)return[];const t=h=>h==='"'||h==="'"||h.charCodeAt(0)===Te,r=(h,g)=>{const b=h[g];if(t(b)){for(g++;g<h.length&&h[g]!==b;)g++;return g}if(b==="["){const x=h.indexOf("]",g);return x<0?h.length:x}return g};let a=0,o=-1;for(let h=n;h<e.length;h++){const g=r(e,h);if(g!==h){h=g;continue}if(e[h]==="(")a++;else if(e[h]===")"&&(a--,a===0)){o=h;break}}if(o<0)return[];const s=e.slice(n+1,o),c=[];let i="",l=0;for(let h=0;h<s.length;h++){const g=r(s,h);if(g!==h){i+=s.slice(h,g+1),h=g;continue}if(s[h]==="("?l++:s[h]===")"&&l--,s[h]===","&&l===0){c.push(i),i="";continue}i+=s[h]}i.trim()&&c.push(i);const d=new Set(["PRIMARY","UNIQUE","CHECK","FOREIGN","CONSTRAINT"]),m=[];for(const h of c){const g=h.trim();if(!g)continue;let b="";if(g.startsWith('"')){const x=g.indexOf('"',1);b=g.slice(1,x>0?x:1)}else if(g.startsWith("[")){const x=g.indexOf("]",1);b=g.slice(1,x>0?x:1)}else if(g.charCodeAt(0)===Te){const x=String.fromCharCode(Te),C=g.indexOf(x,1);b=g.slice(1,C>0?C:1)}else{const x=g.match(/^([A-Za-z_][A-Za-z0-9_]*)/);x&&(b=x[1])}b&&(!g.startsWith('"')&&!g.startsWith("[")&&g.charCodeAt(0)!==Te&&d.has(b.toUpperCase())||m.push(b))}return m}function Ze(e,n,t,r,a,o){for(let s=n;s+8<=n+t;s+=8){const c=r?e.readUInt32BE(s):e.readUInt32LE(s),i=r?e.readUInt32BE(s+4):e.readUInt32LE(s+4);a=a+c+o>>>0,o=o+i+a>>>0}return[a,o]}class dt{constructor(n,t,r){D(this,"fd");D(this,"pageSize");D(this,"reserved");D(this,"wal",new Map);D(this,"cache",new Map);this.fd=n,this.pageSize=t,this.reserved=r}static open(n){const t=f.openSync(n,"r");try{const r=Buffer.alloc(100);let a=0;for(;a<100;){const i=f.readSync(t,r,a,100-a,a);if(i<=0)throw new Error("short header read");a+=i}if(r.toString("latin1",0,16)!=="SQLite format 3\0")throw new Error("not a sqlite database");let o=r.readUInt16BE(16);if(o===1&&(o=65536),o<512||o&o-1)throw new Error("invalid page size");const s=r[20];if(r.readUInt32BE(56)!==1)throw new Error("unsupported text encoding");const c=new dt(t,o,s);return c.loadWal(n+"-wal"),c}catch(r){try{f.closeSync(t)}catch{}throw r}}close(){try{f.closeSync(this.fd)}catch{}}loadWal(n){let t;try{t=f.readFileSync(n)}catch{return}if(t.length<32)return;const r=t.readUInt32BE(0);if(r!==931071618&&r!==931071619)return;const a=r===931071619;if(t.readUInt32BE(8)!==this.pageSize)return;const o=t.readUInt32BE(16),s=t.readUInt32BE(20);let c=0,i=0;if([c,i]=Ze(t,0,24,a,c,i),c!==t.readUInt32BE(24)||i!==t.readUInt32BE(28))return;const l=24+this.pageSize;let d=32;for(;d+l<=t.length;){const m=t.readUInt32BE(d);if(t.readUInt32BE(d+8)!==o||t.readUInt32BE(d+12)!==s||([c,i]=Ze(t,d,8,a,c,i),[c,i]=Ze(t,d+24,this.pageSize,a,c,i),c!==t.readUInt32BE(d+16)||i!==t.readUInt32BE(d+20)))break;m>=1&&this.wal.set(m,t.subarray(d+24,d+24+this.pageSize)),d+=l}}page(n){const t=this.cache.get(n);if(t)return t;const r=this.wal.get(n);if(r)return this.cache.set(n,r),r;if(n>2147483647)return null;const a=Buffer.alloc(this.pageSize);let o=0;try{for(;o<this.pageSize;){const s=f.readSync(this.fd,a,o,this.pageSize-o,(n-1)*this.pageSize+o);if(s<=0)break;o+=s}}catch{return null}return o===0?null:(this.cache.set(n,a),a)}walkTable(n,t,r){if(r>30)return;const a=this.page(n);if(!a)return;const o=n===1?100:0,s=a[o];let c=0;try{c=a.readUInt16BE(o+3)}catch{return}if(s===13){const i=this.pageSize-this.reserved,l=i-35,d=Math.floor((i-12)*32/255)-23;for(let m=0;m<c;m++)try{const h=a.readUInt16BE(o+8+m*2),g=Oe(a,h),b=Oe(a,g.next),x=g.value;let C=x,A=0;if(x>l&&(C=d+(x-d)%(i-4),C>l&&(C=d),A=a.readUInt32BE(b.next+C)),A===0){if(b.next+x>a.length)continue;t(b.value,a.subarray(b.next,b.next+x));continue}const w=Buffer.alloc(x);a.copy(w,0,b.next,b.next+C);let k=C,U=A,S=!1;for(;U&&k<x;){const R=this.page(U);if(!R){S=!0;break}const O=R.readUInt32BE(0),z=Math.min(x-k,i-4);R.copy(w,k,4,4+z),k+=z,U=O}if(S||k!==x)continue;t(b.value,w)}catch{continue}}else if(s===5)try{for(let i=0;i<c;i++){const l=a.readUInt16BE(o+12+i*2);this.walkTable(a.readUInt32BE(l),t,r+1)}this.walkTable(a.readUInt32BE(o+8),t,r+1)}catch{return}}tableRoots(){const n=new Map;return this.walkTable(1,(t,r)=>{const a=Ct(r);a&&a[0]==="table"&&typeof a[1]=="string"&&typeof a[3]=="number"&&typeof a[4]=="string"&&n.set(a[1],{root:a[3],sql:a[4]})},0),n}scanTable(n,t,r,a){const o=Sr(t);if(!o.length)return;const s=[];for(const c of r){const i=o.indexOf(c);i>=0&&s.push([c,i])}this.walkTable(n,(c,i)=>{const l=Ct(i);if(!l)return;const d={};for(const[m,h]of s)d[m]=l[h]??null;a(d)},0)}}const Tt={requests:0,retries:0,toolCalls:0,toolErrors:0,input:0,output:0,reasoning:0,cacheRead:0,cacheWrite:0,total:0,durationMs:0,ttftMs:0};function v(e){return typeof e=="number"&&Number.isFinite(e)?e:0}function L(e){return typeof e=="string"?e:""}function Cr(e){if(!e)return"";const n=new Date(e),t=r=>(r<10?"0":"")+r;return t(n.getHours())+":"+t(n.getMinutes())+":"+t(n.getSeconds())}let $e=null;function Tr(){if($e&&Date.now()-$e.at<6e5)return $e.map;const e=new Map;try{const n=Ne.find(r=>r.id==="zcode"),t=new Set([...((n==null?void 0:n.installPaths)??[]).map(r=>u.join(r,"resources","model-providers")),"D:\\ZCode\\resources\\model-providers"]);for(const r of t){let a=[];try{a=f.readdirSync(r).filter(o=>o.toLowerCase().endsWith(".json"))}catch{continue}for(const o of a)try{const s=JSON.parse(f.readFileSync(u.join(r,o),"utf8"));for(const c of Array.isArray(s==null?void 0:s.providers)?s.providers:[])for(const i of Array.isArray(c==null?void 0:c.models)?c.models:[]){const l=String((i==null?void 0:i.id)??"").trim().toLowerCase(),d=Number(i==null?void 0:i.contextWindow);l&&Number.isFinite(d)&&d>0&&e.set(l,d)}}catch{}}}catch{}return $e={at:Date.now(),map:e},e}function $r(e,n){const t=e.get(n);if(t!==void 0)return t;let r="",a;for(const[o,s]of e)o.length>r.length&&n.startsWith(o+"-")&&(r=o,a=s);return a}function Ye(){return{requests:0,input:0,output:0,reasoning:0,cacheRead:0,cacheWrite:0,total:0,toolCalls:0,retries:0,lastAt:0,lastIn:0,last:null,ctxExc:0}}function Er(e=[]){var r,a;const n=u.join(Re(),"db.sqlite");let t;try{t=dt.open(n)}catch(o){return console.warn("[usage-db] open failed:",o.message),null}try{const o=t.tableRoots(),s=["model_usage","turn_usage","tool_usage","session"];for(const p of s)if(!o.has(p))return console.warn("[usage-db] table missing:",p),null;const c=new Date;c.setHours(0,0,0,0);const i=c.getTime(),l=i+864e5,d=Date.now(),m=new Map,h=new Map,g=new Map,b=new Map,x={requests:0,input:0,output:0,cacheRead:0,cacheWrite:0,reasoning:0,total:0,retries:0},C=o.get("model_usage");t.scanTable(C.root,C.sql,["session_id","turn_id","status","started_at","completed_at","duration_ms","time_to_first_token_ms","first_token_at","output_tokens","input_tokens","reasoning_tokens","cache_read_input_tokens","cache_creation_input_tokens","computed_total_tokens","tool_call_count","retry_count","model_id","query_source","context_exceeded"],p=>{const y=L(p.session_id);if(!y)return;const T=L(p.status),$=v(p.completed_at);if(T==="completed"){const B=L(p.query_source)==="subagent";let E=B?h.get(y):m.get(y);E||(E=Ye(),(B?h:m).set(y,E));const ee=v(p.input_tokens);E.requests++;const V=L(p.turn_id);if(V){let F=g.get(y);F||(F=new Set,g.set(y,F)),F.add(V)}E.input+=ee,E.output+=v(p.output_tokens),E.reasoning+=v(p.reasoning_tokens),E.cacheRead+=v(p.cache_read_input_tokens),E.cacheWrite+=v(p.cache_creation_input_tokens),E.total+=v(p.computed_total_tokens),E.toolCalls+=v(p.tool_call_count),E.retries+=v(p.retry_count),$>E.lastAt&&(E.lastAt=$);const Se=E.last;(!Se||$>=Se.completedAt)&&(E.last={durationMs:v(p.duration_ms),ttftMs:v(p.time_to_first_token_ms),model:L(p.model_id),output:v(p.output_tokens),firstTokenAt:v(p.first_token_at),completedAt:$,turnId:L(p.turn_id)},E.lastIn=ee),$>=i&&$<l&&(x.requests++,x.input+=ee,x.output+=v(p.output_tokens),x.cacheRead+=v(p.cache_read_input_tokens),x.cacheWrite+=v(p.cache_creation_input_tokens),x.reasoning+=v(p.reasoning_tokens),x.total+=v(p.computed_total_tokens),x.retries+=v(p.retry_count))}if(v(p.context_exceeded)===1){const B=$||v(p.started_at);let E=m.get(y);E||(E=Ye(),m.set(y,E)),B>E.ctxExc&&(E.ctxExc=B)}if(T==="running"){const B=v(p.started_at);B>(b.get(y)??0)&&b.set(y,B)}});const A=new Map,w=new Map,k=o.get("turn_usage");t.scanTable(k.root,k.sql,["session_id","turn_id","status","started_at","completed_at","model_request_count","model_retry_count","tool_call_count","tool_error_count","input_tokens","output_tokens","reasoning_tokens","cache_read_input_tokens","cache_creation_input_tokens","computed_total_tokens","duration_ms","time_to_first_token_ms"],p=>{const y=L(p.session_id),T=L(p.turn_id);if(!y||!T)return;const $=L(p.status),B=v(p.completed_at)||v(p.started_at),E=w.get(y);B&&(!E||B>=E.at)&&w.set(y,{status:$,at:B}),$==="completed"&&A.set(y+"|"+T,{requests:v(p.model_request_count),retries:v(p.model_retry_count),toolCalls:v(p.tool_call_count),toolErrors:v(p.tool_error_count),input:v(p.input_tokens),output:v(p.output_tokens),reasoning:v(p.reasoning_tokens),cacheRead:v(p.cache_read_input_tokens),cacheWrite:v(p.cache_creation_input_tokens),total:v(p.computed_total_tokens),durationMs:v(p.duration_ms),ttftMs:v(p.time_to_first_token_ms)})});const U=new Map,S=new Map,R=o.get("tool_usage");t.scanTable(R.root,R.sql,["session_id","tool_name","status","duration_ms","started_at"],p=>{const y=L(p.session_id),T=L(p.status);if(!y)return;if(T==="running"){const ee=v(p.started_at);ee>(S.get(y)??0)&&S.set(y,ee);return}if(T!=="completed"&&T!=="error")return;const $=L(p.tool_name)||"(unknown)";let B=U.get(y);B||(B={total:0,errors:0,list:new Map},U.set(y,B));let E=B.list.get($);E||(E={count:0,durationMs:0,errors:0},B.list.set($,E)),B.total++,E.count++,E.durationMs+=v(p.duration_ms),T==="error"&&(B.errors++,E.errors++)});const O=new Map,z=o.get("session");t.scanTable(z.root,z.sql,["id","title","parent_id","summary_additions","summary_deletions","summary_files"],p=>{const y=L(p.id);y&&O.set(y,{title:L(p.title),parent:typeof p.parent_id=="string"&&p.parent_id?p.parent_id:null,add:typeof p.summary_additions=="number"?p.summary_additions:null,del:typeof p.summary_deletions=="number"?p.summary_deletions:null,files:typeof p.summary_files=="number"?p.summary_files:null})});let K="",q=0;for(const[p,y]of m)y.lastAt>q&&(q=y.lastAt,K=p);const ve=p=>[...m.entries()].filter(([y])=>y.startsWith("sess_subagent")===p).sort((y,T)=>T[1].lastAt-y[1].lastAt).slice(0,6).map(([y])=>y),ae=[],ke=p=>{p&&/^[A-Za-z0-9_-]{1,255}$/.test(p)&&!ae.includes(p)&&ae.push(p)};for(const p of e)ke(p);ke(K),ve(!1).forEach(ke),ve(!0).forEach(ke);const Fe=new Set;for(const p of ae){const y=((a=(r=m.get(p))==null?void 0:r.last)==null?void 0:a.turnId)??"";y&&!A.has(p+"|"+y)&&Fe.add(p+"|"+y)}const ze=new Map;Fe.size&&t.scanTable(C.root,C.sql,["session_id","turn_id","status","input_tokens","output_tokens","computed_total_tokens","duration_ms","cache_read_input_tokens","reasoning_tokens","cache_creation_input_tokens"],p=>{if(L(p.status)!=="completed")return;const y=L(p.session_id)+"|"+L(p.turn_id);if(!Fe.has(y))return;let T=ze.get(y);T||(T={...Tt},ze.set(y,T)),T.requests++,T.input+=v(p.input_tokens),T.output+=v(p.output_tokens),T.total+=v(p.computed_total_tokens),T.durationMs+=v(p.duration_ms),T.cacheRead+=v(p.cache_read_input_tokens),T.reasoning+=v(p.reasoning_tokens),T.cacheWrite+=v(p.cache_creation_input_tokens)});const un=Tr(),ut=p=>{var ft;const y=m.get(p)??Ye(),T=O.get(p),$=y.last;let B=0;if($&&$.output>0){let M=0;$.firstTokenAt&&$.completedAt>$.firstTokenAt?M=$.completedAt-$.firstTokenAt:M=$.durationMs,M>0&&(B=Math.round($.output/(M/1e3)*10)/10)}const E=($==null?void 0:$.turnId)??"",ee=E&&A.get(p+"|"+E)||E&&ze.get(p+"|"+E)||{...Tt},V=U.get(p),Se={total:(V==null?void 0:V.total)??0,errors:(V==null?void 0:V.errors)??0,list:[...(V==null?void 0:V.list.entries())??[]].map(([M,_])=>({name:M,count:_.count,durationMs:_.durationMs,errors:_.errors})).sort((M,_)=>_.count-M.count||(M.name<_.name?-1:1))},F=[...h.entries()].filter(([M])=>{var _;return((_=O.get(M))==null?void 0:_.parent)===p}).sort((M,_)=>_[1].lastAt-M[1].lastAt).map(([M,_])=>{var bt;return{sid:M,title:((bt=O.get(M))==null?void 0:bt.title)??"",task:"",requests:_.requests,total:_.total,input:_.input,output:_.output,cacheRead:_.cacheRead,reasoning:_.reasoning,cacheWrite:_.cacheWrite,last:_.lastAt,active:d-_.lastAt<wr}}),hn={requests:F.reduce((M,_)=>M+_.requests,0),total:F.reduce((M,_)=>M+_.total,0),input:F.reduce((M,_)=>M+_.input,0),output:F.reduce((M,_)=>M+_.output,0),cacheRead:F.reduce((M,_)=>M+_.cacheRead,0),reasoning:F.reduce((M,_)=>M+_.reasoning,0),cacheWrite:F.reduce((M,_)=>M+_.cacheWrite,0),active:F.some(M=>M.active),list:F},ht=(($==null?void 0:$.model)??"").trim().toLowerCase(),gt=ht?$r(un,ht):void 0,qe=S.get(p)??0,Ve=b.get(p)??0,Z=w.get(p);let he="idle",ge=0;return qe&&d-qe<6e5?(he="running",ge=qe):Ve&&d-Ve<6e5?(he="thinking",ge=Ve):Z&&(Z.status==="error"||Z.status==="cancelled")&&d-Z.at<12e3?(he="failed",ge=Z.at):Z&&Z.status==="completed"&&d-Z.at<8e3&&(he="done",ge=Z.at),{sid:p,title:(T==null?void 0:T.title)??"",active:y.lastAt>0&&d-y.lastAt<xr,turns:((ft=g.get(p))==null?void 0:ft.size)??0,requests:y.requests,input:y.input,output:y.output,reasoning:y.reasoning,cacheRead:y.cacheRead,cacheWrite:y.cacheWrite,total:y.total,toolCalls:y.toolCalls,retries:y.retries,ctx:y.lastIn,updated:Cr(y.lastAt),lastAt:y.lastAt,live:{state:he,at:ge},last:{durationMs:($==null?void 0:$.durationMs)??0,ttftMs:($==null?void 0:$.ttftMs)??0,model:($==null?void 0:$.model)??"",tps:B},lastTurn:ee,tools:Se,sub:hn,code:{add:(T==null?void 0:T.add)??null,del:(T==null?void 0:T.del)??null,files:(T==null?void 0:T.files)??null},ctxExc:y.ctxExc,contextWindow:gt??yr,contextAuto:gt!==void 0,isSub:p.startsWith("sess_subagent"),parent:(T==null?void 0:T.parent)??null}},pt=ae.map(ut),pn=K?pt.find(p=>p.sid===K)??ut(K):null;return{v:1,generatedAt:d,today:x,session:pn,recent:pt}}catch(o){return console.warn("[usage-db] snapshot failed:",o.message),null}finally{t.close()}}const $t=400,_r=3e4,Mr=120,N=new Map;function ce(...e){console.error("[usage-pump]",...e)}async function tn(e){var t;const n=((t=W("zcode"))==null?void 0:t.rendererHints)??["out/renderer/index.html","renderer/index.html"];for(const r of n)try{const a=await pe(e,r,{timeoutMs:1500,quiet:!0});if(a.length>0)return a}catch{}try{const a=await(await fetch(`http://127.0.0.1:${e}/json/list`,{signal:AbortSignal.timeout(3e3)})).json();return(Array.isArray(a)?a:[]).filter(We)}catch{return[]}}async function rt(e,n){return await Promise.race([e,new Promise(t=>setTimeout(()=>t(null),n))])}async function Ir(e,n,t=!1){const r=await tn(e);if(r.length===0)return;const a=[];for(const d of r){const m=new X(d.webSocketDebuggerUrl);try{await m.open();const h=await rt(m.evaluate("(window.__dreamWorkUsageWant || '')"),1500),g=typeof h=="string"?h.trim():"";g&&/^[A-Za-z0-9_-]{1,255}$/.test(g)&&!a.includes(g)&&a.push(g)}catch{}finally{m.close()}}const o=vr(),s=a.join(",");if(!t&&s===n.lastWants&&o>0&&o===n.lastStamp)return;const c=Date.now();if(c-n.lastSpawnAt<$t){Pr(e,$t-(c-n.lastSpawnAt)+50);return}n.lastWants=s,o&&(n.lastStamp=o),n.lastSpawnAt=c;const i=Er(a);if(!i)return;const l="window.__dreamWorkUsageUpdate && window.__dreamWorkUsageUpdate("+JSON.stringify(i).replace(/[\u2028\u2029]/g,d=>d==="\u2028"?"\\u2028":"\\u2029")+")";for(const d of r){const m=new X(d.webSocketDebuggerUrl);try{await m.open(),await rt(m.evaluate(l),5e3)}catch{}finally{m.close()}}t||(n.settleTimer&&clearTimeout(n.settleTimer),n.settleTimer=setTimeout(()=>{const d=N.get(e);d&&(d.settleTimer=null),oe(e,!0)},2500))}function Pr(e,n){const t=N.get(e);t&&(t.retryTimer&&clearTimeout(t.retryTimer),t.retryTimer=setTimeout(()=>{const r=N.get(e);r&&(r.retryTimer=null,oe(e))},Math.max(50,n)))}let Qe=!1;async function oe(e,n=!1){const t=N.get(e);if(!(!t||t.busy||Qe)){t.busy=!0,Qe=!0;try{await Ir(e,t,n)}catch(r){ce("cycle failed:",r.message)}finally{t.busy=!1,Qe=!1}}}function Ue(e){const n=N.get(e);if(!(!n||n.watcher))try{n.watcher=f.watch(Re(),(t,r)=>{if(r&&!/db\.sqlite/.test(r))return;const a=N.get(e);a&&(a.debounceTimer&&clearTimeout(a.debounceTimer),a.debounceTimer=setTimeout(()=>{const o=N.get(e);o&&(o.debounceTimer=null,oe(e))},Mr))}),n.watcher.on("error",()=>{ce("db watcher error, re-arm in 5s");const t=N.get(e);if(t!=null&&t.watcher){try{t.watcher.close()}catch{}t.watcher=null}setTimeout(()=>{N.has(e)&&Ue(e)},5e3)}),ce("watching",Re(),"for port",e)}catch(t){ce("fs.watch failed, retry in 5s:",t.message),setTimeout(()=>{N.has(e)&&Ue(e)},5e3)}}let Ie=null;async function Ar(e){const n=N.get(e);if(!n||n.busy)return;const t=await tn(e).catch(()=>[]),r=t&&t[0];if(!r)return;const a=new X(r.webSocketDebuggerUrl);try{await a.open();const o=await rt(a.evaluate("(window.__dreamWorkUsageWant || '')"),1200),s=typeof o=="string"?o.trim():"";s&&/^[A-Za-z0-9_-]{1,255}$/.test(s)&&!n.lastWants.split(",").includes(s)&&oe(e)}catch{}finally{a.close()}}function nn(e){let n=N.get(e);if(n){n.watcher||Ue(e);return}n={watcher:null,heartbeat:null,retryTimer:null,debounceTimer:null,settleTimer:null,busy:!1,lastSpawnAt:0,lastStamp:0,lastWants:""},N.set(e,n),Ue(e),n.heartbeat=setInterval(()=>void oe(e,!0),_r),Ie=setInterval(()=>void Ar(e),2e3),[3e3,8e3,15e3].forEach(t=>setTimeout(()=>{N.has(e)&&oe(e,!0)},t)),ce("started for port",e,"(forced heartbeat + startup ramp + realtime 120ms, 0.7.13)"),oe(e)}function Rr(e){const n=N.get(e);if(n){if(N.delete(e),n.watcher)try{n.watcher.close()}catch{}n.heartbeat&&clearInterval(n.heartbeat),n.retryTimer&&clearTimeout(n.retryTimer),n.debounceTimer&&clearTimeout(n.debounceTimer),n.settleTimer&&clearTimeout(n.settleTimer),Ie&&(clearInterval(Ie),Ie=null),ce("stopped for port",e)}}function Or(e){return N.has(e)}const Q="dream-work-style",H="dream-work-menu",le=new Map,be=new Map,ne=new Map,xe=new Map,I={id:"wb-dream-sentinel-id",hero:"data:image/png;base64,WBDREAMHEROSENTINEL",accent:"#010203",secondary:"#040506",surface:"#070809",text:"#0a0b0c",textSubtle:"#0d0e0f",textSubtlest:"#101112",textSecondary:"#131415"},Ur={zcode:[.6,.66,.78,.8],codex:[.76,.82,.86,.9,.92],catpaw:[.78,.82],"qoder-work":[.7,.82,.86,.9],"qwen-office":[.86,.9],workbuddy:[.58,.62,.92],"hana-agent":[.62,.66,.78]},rn=[.6,.66,.78,.8];function at(e){return Ur[e]??rn}function an(e){if(e)return"file:///"+encodeURI(e.replace(/\\/g,"/")).replace(/#/g,"%23").replace(/\?/g,"%3F")}let Ee=null;function Br(){if(Ee)return Ee.length?Ee:void 0;const e=[];try{const n=P.app.getAppPath(),t=a=>n.endsWith(".asar")&&(a===n||a.startsWith(n+u.sep)),r=u.resolve(u.join(n,"assets","pet"));for(const a of f.readdirSync(r)){if(a!==u.basename(a))continue;const o=u.resolve(r,a);if(o!==r&&!o.startsWith(r+u.sep))continue;try{if(!f.statSync(o).isDirectory())continue}catch{continue}let s={};try{s=JSON.parse(f.readFileSync(u.join(o,"pet.json"),"utf8"))}catch{}const c={};try{for(const i of f.readdirSync(o)){const l=u.extname(i).toLowerCase();if(![".gif",".webp",".png"].includes(l)||i!==u.basename(i))continue;const d=i.slice(0,-l.length);if(c[d]&&l!==".webp")continue;let m=u.join(o,i);if(t(m)){const b=u.resolve(n+".unpacked",u.relative(n,m));f.existsSync(b)&&(m=b)}const h=an(m);if(!h)continue;let g=0;try{g=Math.round(f.statSync(m).mtimeMs)}catch{}c[d]=g?h+"?v="+g:h}}catch{}Object.keys(c).length&&e.push({id:String(s.id||a),name:String(s.name||a),order:Number.isFinite(Number(s.order))?Number(s.order):99,scale:Number.isFinite(Number(s.scale))&&Number(s.scale)>0?Number(s.scale):.5,states:c})}e.sort((a,o)=>a.order-o.order||(a.id<o.id?-1:1))}catch(n){console.warn("[injector] pet registry unavailable:",n.message)}return Ee=e,e.length?e:void 0}const Et=new Map;function Dr(e){let n;try{n=f.statSync(e)}catch{return null}const t=Et.get(e);if(t&&t.size===n.size&&t.mtimeMs===n.mtimeMs)return t.rgb;let r=null;try{const a=P.nativeImage.createFromPath(e);if(!a.isEmpty()){const o=a.resize({width:1,height:1}).toDataURL();r=fr(Buffer.from(o.slice(o.indexOf(",")+1),"base64"))}}catch(a){console.warn("[injector] Hero average sampling failed:",a.message)}return Et.set(e,{size:n.size,mtimeMs:n.mtimeMs,rgb:r}),r}let _e=null;async function Lr(){if(!_e)try{const e=u.resolve(__dirname,"manager","codex-dream-skin.css");_e=await kn.readFile(e,"utf-8")}catch(e){console.warn("[injector] Failed to load Codex base CSS:",e.message),_e=""}return _e}async function on(e,n,t,r={}){const a=W(e),o=r.rendererUrlHint?[r.rendererUrlHint]:(a==null?void 0:a.rendererHints)??["renderer/index.html","index.html"];let s=[],c="No renderer targets found";for(const i of o)try{if(console.log(`[injector] Trying hint "${i}" on port ${t}`),s=await Qn(t,i,{timeoutMs:2e4,pollMs:500}),s.length>0){console.log(`[injector] Found ${s.length} targets with hint "${i}"`);break}}catch(l){c=l.message,console.log(`[injector] Hint "${i}" failed: ${l.message}`)}if(s.length===0)try{console.log(`[injector] Strict hints failed, trying relaxed page-target fallback on port ${t}`);const l=await(await fetch(`http://127.0.0.1:${t}/json/list`,{signal:AbortSignal.timeout(5e3)})).json(),d=(Array.isArray(l)?l:[]).filter(We).sort((m,h)=>{const g=[String(m.id??""),m.url,m.webSocketDebuggerUrl],b=[String(h.id??""),h.url,h.webSocketDebuggerUrl];for(let x=0;x<g.length;x++){if(g[x]<b[x])return-1;if(g[x]>b[x])return 1}return 0});d.length>0&&(console.log(`[injector] Relaxed fallback found ${d.length} page targets`),s=d)}catch(i){console.log(`[injector] Relaxed fallback failed: ${i.message}`)}if(s.length===0)return{success:!1,applied:0,error:c};try{const i=He(e);if(console.log(`[injector] Loaded ${i.length} themes`),!i.some(w=>w.id===n))return{success:!1,applied:0,error:`Theme ${n} is not compatible with ${e}`};const l=pr(e,i.map(w=>w.id),n,8),d=new Map(i.map(w=>[w.id,w])),m=l.map(w=>d.get(w)).filter(Boolean),h=new Map;for(const w of m){const k=e==="zcode"?dr(w):null;h.set(w.id,{name:w.name,css:Rt(e,w.manifest,sr(w),Dr(u.join(w.path,w.manifest.hero)),{video:!!k}),surface:w.manifest.colors.surface,videoUrl:an(k)})}const g=Array.from(h.entries()).map(([w,k])=>{var U;return{id:w,name:k.name,css:k.css,surface:k.surface,accent:((U=i.find(S=>S.id===w))==null?void 0:U.manifest.colors.accent)??"#24c9d7",videoUrl:k.videoUrl}});let b=ct();if(b.length===0){const w=e==="workbuddy"?"dreamCustomThemes":"dreamCodexCustomThemes";for(const k of s){const U=new X(k.webSocketDebuggerUrl);try{await U.open();const S=await U.evaluate(`(() => localStorage.getItem(${JSON.stringify(w)}) || '[]')()`),R=JSON.parse(S);if(Array.isArray(R)&&R.length>0){b=ur(R);break}}catch(S){console.warn(`[injector] Failed to import existing custom themes from ${e} target ${k.id}:`,S)}finally{U.close()}}}const x=await hr(),C=e==="workbuddy"?oa({styleId:Q,menuId:H,currentThemeId:n,themes:g,sharedCustomThemes:b,sharedCustomThemeService:x,cssTemplate:cn({id:I.id,colors:{accent:I.accent,secondary:I.secondary,surface:I.surface,text:I.text},copy:null},I.hero,{accent:I.accent,secondary:I.secondary,surface:I.surface,text:I.text})}):e==="hana-agent"?ea({styleId:Q,menuId:H,currentThemeId:n,themes:g,sharedCustomThemes:b,sharedCustomThemeService:x,cssTemplate:sn({id:I.id,colors:{accent:I.accent,secondary:I.secondary,surface:I.surface,text:I.text}},I.hero,{accent:I.accent,secondary:I.secondary,surface:I.surface,text:I.text})}):sa({styleId:Q,menuId:H,currentThemeId:n,appId:e,themes:g,sharedCustomThemes:b,sharedCustomThemeService:x,cssTemplate:Rt(e,{id:I.id,colors:{accent:I.accent,secondary:I.secondary,surface:I.surface,text:I.text}},I.hero,null,{template:!0}),surfaceAlphas:at(e),pets:e==="zcode"?Br():void 0});let A=0;for(const w of s)try{console.log(`[injector] Injecting to target ${w.id}: ${w.url}`);const k=new X(w.webSocketDebuggerUrl);if(await k.open(),e==="workbuddy"&&!await k.evaluate(`(() => {
          ...
          })()`)){console.warn(`[injector] Skipping non-WorkBuddy target ${w.id}: ${w.url}`),k.close();continue}if(e==="codex"){const S=await Lr();S&&await k.evaluate(`(() => {
          ...
          })()`,R=le.get(w.id);R&&await k.removeScriptToEvaluateOnNewDocument(R).catch(()=>{});const O=await k.addScriptToEvaluateOnNewDocument(S);O&&le.set(w.id,O)}const U=await k.evaluate(e==="hana-agent"?`(() => { window.__dreamWorkForceApply = true; return ${C}; })()`:C);if(console.log(`[injector] Injection result for target ${w.id}:`,U),e==="zcode"){const S=br(),R=xe.get(w.id);R&&await k.removeScriptToEvaluateOnNewDocument(R).catch(()=>{});const O=await k.addScriptToEvaluateOnNewDocument(`(() => { const inject = () => { ${S} }; if (document.readyState === 'loading') window.addEventListener('DOMContentLoaded', inject, { once: true }); else inject(); })()`).catch(()=>{});O&&xe.set(w.id,O),await k.evaluate(S).catch(z=>console.warn(`[injector] Usage bar injection failed for target ${w.id}:`,z.message))}if(e==="hana-agent"){let S=!1;for(let R=0;R<20&&(S=await k.evaluate(`(() => {
            ...
            })()`).catch(()=>!1),!S);R++)await new Promise(O=>setTimeout(O,100));if(!S){console.warn(`[injector] HanaAgent injection did not become ready for target ${w.id}`),k.close();continue}}if(e==="codex")for(let S=1;S<=4;S++){const R=await k.evaluate(`(() => {
            ...
            })`);if(R.homeClasses&&R.homeClasses.includes("dream-skin-home")){console.log(`[injector] Codex home detection for ${w.id}: attempt=${S}`,JSON.stringify(R));break}S<4&&await new Promise(O=>setTimeout(O,800))}if(e==="codex")try{const S=await k.evaluate(`(() => {
            ...
            })()`);console.log(`[injector] Codex debug info for ${w.id}:`,JSON.stringify(S,null,2))}catch(S){console.error(`[injector] Failed to get debug info for ${w.id}:`,S)}k.close(),A++}catch(k){console.error(`[injector] Failed to inject to target ${w.id}:`,k)}if(e==="hana-agent"&&A>0){const w=new Set(s.map(R=>R.id)),k=Date.now()+2e4;let U="",S=0;for(;Date.now()<k;){let R=[];try{R=await pe(t,".hanako/artifacts/renderer/",{timeoutMs:2e3,quiet:!0})}catch{}const O=R[0];if(!O){U="",S=0,await new Promise(q=>setTimeout(q,250));continue}if(!w.has(O.id)){console.log(`[injector] HanaAgent created renderer target ${O.id}; injecting theme`);const q=new X(O.webSocketDebuggerUrl);try{await q.open();const ve=`(() => {
              const inject = () => ${C};
              if (document.readyState === 'loading') window.addEventListener('DOMContentLoaded', inject, { once: true });
              else inject();
            })()`,ae=await q.addScriptToEvaluateOnNewDocument(ve);ae&&le.set(O.id,ae),await q.evaluate(`(() => { window.__dreamWorkForceApply = true; return ${C}; })()`),w.add(O.id)}finally{q.close()}}const z=new X(O.webSocketDebuggerUrl);let K=!1;try{await z.open(),K=await z.evaluate(`(() => {
            const host = document.getElementById('${H}-host');
            return Boolean(document.getElementById('${Q}') && host?.shadowRoot?.getElementById('${H}') && document.documentElement.dataset.dreamTheme);
          })()`)}catch{}finally{z.close()}if(K){if(U!==O.id)U=O.id,S=Date.now();else if(Date.now()-S>=2e3)return jr(t,C,w),nt(e,n),{success:!0,applied:1}}else U="",S=0;await new Promise(q=>setTimeout(q,250))}return{success:!1,applied:0,error:"HanaAgent renderer did not stabilize with the injected theme"}}return e==="zcode"&&A>0&&nn(t),A>0&&nt(e,n),{success:A>0,applied:A}}catch(i){return console.error("[injector] Injection failed:",i),{success:!1,applied:0,error:i.message}}}async function Nr(e,n,t={}){return Hr(e,n,t)}function jr(e,n,t){const r=be.get(e);r&&clearInterval(r);const a=(ne.get(e)??0)+1;ne.set(e,a);let o=!1;const s=setInterval(async()=>{if(!o&&ne.get(e)===a){o=!0;try{const i=(await pe(e,".hanako/artifacts/renderer/",{timeoutMs:1e3,quiet:!0}))[0];if(!i||ne.get(e)!==a)return;const l=new X(i.webSocketDebuggerUrl);try{await l.open();const d=await l.evaluate(`(() => {
          const host = document.getElementById('${H}-host');
          ...
          return document.getElementById('${Q}') && host?.shadowRoot?.getElementById('${H}') && document.documentElement.dataset.dreamTheme
            ? 'ready'
            : 'missing';
        })()`).catch(()=>"missing");if(d==="ready"||d==="restored"){t.add(i.id);return}if(console.log(`[injector] HanaAgent watcher restoring theme on renderer target ${i.id}`),ne.get(e)!==a)return;const m=`(() => {
          const inject = () => ${n};
          if (document.readyState === 'loading') window.addEventListener('DOMContentLoaded', inject, { once: true });
          else inject();
        })()`;if(!t.has(i.id)){const h=await l.addScriptToEvaluateOnNewDocument(m);h&&le.set(i.id,h)}if(await l.evaluate(n),ne.get(e)!==a){await l.evaluate(`(() => {
          ...
          })()`).catch(()=>{});return}t.add(i.id)}finally{l.close()}}catch{await Wr(e)||(clearInterval(s),be.delete(e))}finally{o=!1}}},1e3);be.set(e,s)}async function Wr(e){try{return(await fetch(`http://127.0.0.1:${e}/json/version`,{signal:AbortSignal.timeout(500)})).ok}catch{return!1}}async function Hr(e,n,t={}){var c;const r=t.rendererUrlHint?[t.rendererUrlHint]:((c=W(e))==null?void 0:c.rendererHints)??["renderer/index.html","index.html"];let a=[];for(const i of r)try{if(a=await pe(n,i,{timeoutMs:1e3,quiet:!0}),a.length>0)break}catch{}if(a.length===0)try{const l=await(await fetch(`http://127.0.0.1:${n}/json/list`,{signal:AbortSignal.timeout(5e3)})).json();a=(Array.isArray(l)?l:[]).filter(We).sort((d,m)=>{const h=[String(d.id??""),d.url,d.webSocketDebuggerUrl],g=[String(m.id??""),m.url,m.webSocketDebuggerUrl];for(let b=0;b<h.length;b++){if(h[b]<g[b])return-1;if(h[b]>g[b])return 1}return 0})}catch{}if(a.length===0)return{installed:!1,menu:!1,targets:0};const o=[];for(const i of a){const l=new X(i.webSocketDebuggerUrl);try{if(await l.open(),e==="workbuddy"&&!await l.evaluate("(() => document.body?.dataset.applicationName === 'workbuddy')()"))continue;const d=await l.evaluate(`(() => {
      ...
      })()`),m=JSON.parse(d);o.push(m)}catch(d){console.warn(`[injector] Status check failed for ${e} target ${i.id}:`,d)}finally{l.close()}}const s=o.find(i=>i.installed&&i.themeId)??o.find(i=>i.installed);return{installed:o.some(i=>i.installed),menu:o.some(i=>i.menu),themeId:s==null?void 0:s.themeId,targets:o.length}}async function Fr(e,n,t={}){var o;if(e==="hana-agent"){ne.set(n,(ne.get(n)??0)+1);const s=be.get(n);s&&clearInterval(s),be.delete(n)}const r=t.rendererUrlHint??((o=W(e))==null?void 0:o.rendererHints[0])??"renderer/index.html";let a=[];try{a=await pe(n,r)}catch{}if(a.length===0)try{const c=await(await fetch(`http://127.0.0.1:${n}/json/list`,{signal:AbortSignal.timeout(5e3)})).json();a=(Array.isArray(c)?c:[]).filter(We).sort((i,l)=>{const d=[String(i.id??""),i.url,i.webSocketDebuggerUrl],m=[String(l.id??""),l.url,l.webSocketDebuggerUrl];for(let h=0;h<d.length;h++){if(d[h]<m[h])return-1;if(d[h]>m[h])return 1}return 0})}catch{}if(a.length===0)return{success:!1};for(const s of e==="hana-agent"?a:a.slice(0,1)){const c=new X(s.webSocketDebuggerUrl);if(await c.open(),e==="hana-agent"){const i=le.get(s.id);i&&(await c.removeScriptToEvaluateOnNewDocument(i).catch(()=>{}),le.delete(s.id))}if(e==="zcode"){const i=xe.get(s.id);i&&(await c.removeScriptToEvaluateOnNewDocument(i).catch(()=>{}),xe.delete(s.id))}await c.evaluate(`(() => {
      ...
      return true;
    })`),c.close()}return e==="zcode"&&(xe.clear(),Rr(n)),{success:!0}}const zr=.9,qr=.92,Me=.45;function Be(e,n,t){return me((e%1+1)%1,n,Math.max(.02,Math.min(.98,t)))}const Vr=.2;function _t(e,n,t=Vr){if(!n.length)return e;const r=de(e),a=n.reduce((l,d)=>l+de(d),0)/n.length,[o,s,c]=J(e),i=r>=a?c+(1-c)*t:c*(1-t);return me(o,s,Math.max(.02,Math.min(.99,i)))}const Gr=30;function Mt(e,n,t,r=Gr){const a=m=>Math.max(m[0],m[1],m[2])-Math.min(m[0],m[1],m[2]),o=m=>n.length?Math.min(...n.map(h=>re(m,h))):99;if(a(e)>=r)return e;const[s,c,i]=J(e),l=i>.5?-1:1;let d=e;for(let m=1;m<=20;m++){const h=me(s,Math.min(1,c+.06*m),Math.max(.06,Math.min(.99,i+l*.02*m)));if(o(h)<t||(d=h,a(h)>=r))break}return d}const It=.7;function Kr(e,n,t,r){let a,o=.58;try{a=se(n),o=J(se(e))[0]}catch{a=[12,12,18]}const s=t??a,c=r.map(m=>Ae(s,a,m)),i=m=>Math.min(...c.map(h=>re(m,h))),l=Be(o,It,.92),d=Be(o,It,.12);return Y(i(l)>=i(d)?l:d)}function Jr(e,n,t,r,a,o){const s=se(e),c=r.map(w=>Ae(t??s,s,w)),i=(w,k)=>{try{return w?se(w):k??[255,255,255]}catch{return k??[255,255,255]}},l=i(n,[255,255,255]),d=i(a,l),m=i(o,d),h=J(d)[0];J(m)[0];const g=J(l)[2],b=St(Be(h,zr,g),c,4.5),x=(w,k,U,S,R,O=0)=>{const[z]=J(w),K=((z+O)%1+1)%1,q=_t(St(Be(K,k,U),c,S),c);return Mt(q,c,S,R)},C=Mt(_t(b,c),c,4.5,60),A=J(C)[2];return{text:Y(C),textVivid:Y(x(d,qr,g,4.5,80)),textAlt:Y(x(m,Math.max(Me+.25,.7),g,3,45)),textOpposite:Y(x(d,.95,g,3,90,.5)),textSubtle:Y(x(d,Me,A<.5?Math.min(.98,A+.3):Math.max(.02,A-.26),3,34)),textSubtlest:Y(x(d,Me*.8,A<.5?Math.min(.98,A+.44):Math.max(.02,A-.38),3,30)),textSecondary:Y(x(d,Me*.9,A<.5?Math.min(.98,A+.36):Math.max(.02,A-.32),3,32))}}function Xr(e){const n=e[0]/255,t=e[1]/255,r=e[2]/255,a=Math.max(n,t,r),o=Math.min(n,t,r),s=(a+o)/2,c=a-o,i=c===0?0:c/(1-Math.abs(2*s-1)),m=(((c===0?0:a===n?(t-r)/c%6:a===t?(r-n)/c+2:(n-t)/c+4)*60%360+360)%360-60+360)%360,h=(1-Math.abs(2*s-1))*Math.max(i,.35),g=h*(1-Math.abs(m/60%2-1)),b=s-h/2;return(m<60?[h,g,0]:m<120?[g,h,0]:m<180?[0,h,g]:m<240?[0,g,h]:m<300?[g,0,h]:[h,0,g]).map(C=>(C+b)*255)}function Pt(e){let n;try{n=se(e)}catch{return null}const t=n[0]/255,r=n[1]/255,a=n[2]/255,o=Math.max(t,r,a),s=Math.min(t,r,a),c=o-s;let i=0;return c>0&&(i=((o===t?(r-a)/c%6:o===r?(a-t)/c+2:(t-r)/c+4)*60%360+360)%360),{h:i,chroma:c}}function At(e,n){const t=Pt(e),r=Pt(n);if(!t||!r)return n;const a=Math.abs(t.h-r.h)%360;return(a>180?360-a:a)>=45&&r.chroma>=.1?n:Y(Xr(se(e)))}function Rt(e,n,t,r=null,a={}){var m,h,g,b,x,C;const o=((m=n.colors)==null?void 0:m.surface)??"#f7fbff",s=((h=n.colors)==null?void 0:h.text)??"#17344f",c=((g=n.colors)==null?void 0:g.accent)??"#24c9d7",i=a.template?{text:s,textSubtle:I.textSubtle,textSubtlest:I.textSubtlest,textSecondary:I.textSecondary}:Jr(o,a.video?Kr(c,o,r,at(e)):s,r,at(e),c,e==="zcode"&&!a.template?At(c,((b=n.colors)==null?void 0:b.secondary)??"#ef8fd3"):((x=n.colors)==null?void 0:x.secondary)??"#ef8fd3"),l={accent:c,secondary:((C=n.colors)==null?void 0:C.secondary)??"#ef8fd3",surface:o,...i};if(e==="zcode"&&!a.template&&(l.secondary=At(l.accent,l.secondary)),e==="codex")return ra(n,t,l);const d=W(e);return(d==null?void 0:d.kind)==="vscode-work"?Zr(n,t,l):(d==null?void 0:d.kind)==="generic-work"?e==="hana-agent"?sn(n,t,l):Yr(e,n,t,l,!!a.video):cn({...n,copy:null},t,l)}function Zr(e,n,t){return`/* DREAM_THEME:${e.id} */
:root {
...
:root {
  ...
  --dream-work-text-vivid: ${r.textVivid??r.text};
  --dream-work-text-alt: ${r.textAlt??r.textSecondary??r.text};
  /* 相反色相档：思考行与工具输出标签（accent +180°、高饱和） */
  --dream-work-text-opposite: ${r.textOpposite??r.textVivid??r.text};
  /* 正文技术记号的"纯黑/纯白"档：亮主题纯黑、暗主题纯白（按正文极性的明度判定） */
  --dream-work-ink: ${(()=>{try{const g=se(r.text);return .2126*g[0]+.7152*g[1]+.0722*g[2]<96?"#000000":"#ffffff"}catch{return"#ffffff"}})()};
  ...
  --agents-sidebar-material-bg: color-mix(in srgb, ${r.surface} 80%, transparent) !important;
  --text-base-primary: ${r.text} !important;
  --text-base-secondary: ${r.textSecondary} !important;
  --bg-base: color-mix(in srgb, ${r.surface} 76%, transparent) !important;
...
:is(${i}) {
  background: color-mix(in srgb, ${r.surface} 82%, transparent) !important;
...
:is(${c}) :where(p, span, li, h1, h2, h3, h4, strong, em, code, pre, kbd, samp, time, small, b, i, u, del, ins, mark) { color: ${r.text} !important; }
...
#dream-work-video-layer[data-motion-error="true"] video { display: none !important; }`:""}`}function Qr(e,n=!1){return`
...
`}function Ot(e,n=""){return JSON.stringify(typeof e=="string"?e:n)}function cn(e,n,t){var a,o;return`/* DREAM_THEME:${String(e.id??"custom").replace(/[^a-z0-9_-]/gi,"")} */
...
#root::before {
  ...
  top: 60px;
  left: max(300px, 22vw);
  content: ${Ot((a=e.copy)==null?void 0:a.brand)};
  color: var(--wb-accent);
...
#root::after {
  ...
  left: max(300px, 22vw);
  max-width: 42vw;
  content: ${Ot((o=e.copy)==null?void 0:o.headline)};
  color: var(--wb-text);
  font: 750 clamp(18px, 2.7vw, 42px)/1.15 ui-rounded, system-ui;
  text-shadow: 0 2px 12px white;
  pointer-events: none;
}`}function ra(e,n,t){const r=aa(t.surface),a=r?`color-mix(in srgb, ${t.surface} 80%, transparent)`:`color-mix(in srgb, ${t.surface} 76%, transparent)`,o=r?`color-mix(in srgb, ${t.accent} 16%, ${t.surface})`:`color-mix(in srgb, ${t.accent} 42%, ${t.surface})`,s=r?"#172033":`color-mix(in srgb, ${t.surface} 62%, #000000)`,c="#f2f6ff",i=`/* DREAM_THEME:${e.id} */
...
`+l}function aa(e){const n=/^#([0-9a-f]{6})$/i.exec(e);if(!n)return!0;const t=parseInt(n[1],16);return .299*(t>>16&255)+.587*(t>>8&255)+.114*(t&255)>140}function oa(e){return`(() => {
  ...
  return true;
})()`}function sa(e){const n=JSON.stringify(e.themes),t=JSON.stringify(e.cssTemplate??""),r=e.appId,a=e.surfaceAlphas??rn;return`(() => {
  const themes = ${n};
  const cssTemplate = ${t};
  const sentinels = ${JSON.stringify(I)};
  ...
  ensureInjectedNodes();
    ...
    $Shortcut.Save()
  `;return new Promise(s=>{require("child_process").execFile("powershell.exe",["-NoProfile","-Command",o],c=>{s(c?{success:!1,error:c.message}:{success:!0,path:t})})})}function da(e){const n=u.join(j.homedir(),"Desktop"),t=u.join(n,`${mt(e.label)}.app`),a=`
    tell application "Terminal"
      do script "'${process.execPath}' --launch=${e.appId}:${e.themeId}"
    end tell
  `,o=u.join(n,`${e.id}.scpt`);return f.writeFileSync(o,a),new Promise(s=>{require("child_process").execFile("osacompile",["-o",t,o],c=>{f.unlinkSync(o),s(c?{success:!1,error:c.message}:{success:!0,path:t})})})}async function ma(e){const n=u.join(j.homedir(),".local","share","applications");f.existsSync(n)||f.mkdirSync(n,{recursive:!0});const t=u.join(n,`${e.id}.desktop`),r=process.execPath,a=`[Desktop Entry]
Type=Application
Name=${mt(e.label)}
...
Terminal=false
Categories=Utility;
`;return f.writeFileSync(t,a),f.chmodSync(t,493),{success:!0,path:t}}const ua=ot.promisify(Le.execFile),pa="https://api.dreamskin.cc",ln=`${pa}/v1/themes`,dn=32*1024*1024,De=6;let et=0;const ha=["workbuddy","codex","trae-work","qoder-work","catpaw","zcode","qwen-office","hana-agent"];async function ga(){const e=et,n=await fa(e),t=n.items;et=e+t.length>=n.total?0:e+De;const r=Gt(),a={checked:t.length,imported:0,skipped:0,offset:e,page:Math.floor(e/De)+1,total:n.total,nextOffset:et,failed:[]};for(const o of t){const s=va(o.themeId);if(!o.applyCompatible||Kt(s)){a.skipped++;continue}try{await ba(o,r,s)?a.imported++:a.skipped++}catch(c){a.failed.push({id:o.id,name:o.name,error:c.message})}}return a}async function fa(e){const n=`${ln}?limit=${De}&offset=${e}&sort=recent`,t=await fetch(n,{signal:AbortSignal.timeout(3e4),redirect:"error"});if(!t.ok)throw new Error(`Theme list request failed: HTTP ${t.status}`);const r=await t.json();if(!Array.isArray(r.items)||r.items.length>De||!Number.isInteger(r.total)||r.total<0)throw new Error("Theme list response is invalid");return{items:r.items,total:r.total}}async function ba(e,n,t){ya(e);const r=f.mkdtempSync(u.join(j.tmpdir(),"dream-work-theme-")),a=u.join(r,"theme.zip"),o=u.join(r,"extract"),s=u.join(n,`.updating-${t}-${process.pid}`);try{f.mkdirSync(o);const c=`${ln}/${e.id}/download`,i=await fetch(c,{signal:AbortSignal.timeout(12e4),redirect:"error"});if(!i.ok)throw new Error(`Theme download failed: HTTP ${i.status}`);const l=Buffer.from(await i.arrayBuffer());if(l.length!==e.packageBytes)throw new Error(`Downloaded size mismatch: expected ${e.packageBytes}, got ${l.length}`);if(l.length>dn)throw new Error("Theme package exceeds 32 MiB");if(st.createHash("sha256").update(l).digest("hex")!==e.packageSha256)throw new Error("Downloaded SHA-256 does not match metadata");f.writeFileSync(a,l,{flag:"wx"}),await xa(a,o);const m=wa(o),h=JSON.parse(f.readFileSync(u.join(m,"theme.json"),"utf8")),g=h.image;if(typeof g!="string"||u.basename(g)!==g||!/\.(png|jpe?g|webp)$/i.test(g))throw new Error("Theme image name is invalid");const b=u.join(m,g),x=u.join(m,"theme.css");if(!f.existsSync(b)||!f.statSync(b).isFile())throw new Error("Theme image is missing");if(!f.existsSync(x)||!f.statSync(x).isFile())throw new Error("theme.css is missing");const C=ka(h,e,t,`hero${u.extname(g).toLowerCase()}`);return ir(C.name,C.author,b)?!1:(f.mkdirSync(s),f.copyFileSync(b,u.join(s,C.hero)),f.copyFileSync(x,u.join(s,"theme.css")),f.writeFileSync(u.join(s,"theme.json"),`${JSON.stringify(C,null,2)}
`),f.renameSync(s,u.join(n,t)),!0)}finally{f.rmSync(s,{recursive:!0,force:!0}),f.rmSync(r,{recursive:!0,force:!0})}}async function xa(e,n){const{path7za:t}=require("7zip-bin");await ua(t,["x",e,`-o${n}`,"-y"],{windowsHide:!0,timeout:12e4})}function wa(e){const t=[e,...f.readdirSync(e,{withFileTypes:!0}).filter(r=>r.isDirectory()).map(r=>u.join(e,r.name))].filter(r=>f.existsSync(u.join(r,"theme.json"))&&f.existsSync(u.join(r,"theme.css")));if(t.length!==1)throw new Error("Theme ZIP must contain one theme root");return t[0]}function ya(e){if(!/^ver_[a-z0-9]{8,64}$/.test(e.id))throw new Error("Theme version ID is invalid");if(!Number.isInteger(e.packageBytes)||e.packageBytes<1||e.packageBytes>dn)throw new Error("Theme package size is invalid");if(!/^[a-f0-9]{64}$/.test(e.packageSha256))throw new Error("Theme package SHA-256 is invalid")}function va(e){return String(e).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").replace(/-+/g,"-")||"community-theme"}function ka(e,n,t,r){const a=e.appearance==="dark"?"dark":"light",o=a==="dark"?"#10141c":"#f4f7fa",s=e.colors||{};return{schemaVersion:1,id:t,name:String(e.name||n.name||t).trim(),author:n.authorDisplayName||"DreamSkin Community",hero:r,colors:{accent:fe(s.accent,"#4f8cff",o),secondary:fe(s.secondary||s.accentAlt,"#7ba7d8",o),surface:fe(s.panelAlt||s.panel||s.background,o,o),text:fe(s.text,a==="dark"?"#eef2f7":"#1f2937",o)},copy:null,apps:Object.fromEntries(ha.map(c=>[c,{compat:!0}]))}}function fe(e,n,t){if(typeof e!="string")return n;const r=e.trim().match(/^#([0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i);if(r){let i=r[1];return i.length===3&&(i=i.split("").map(l=>l+l).join("")),`#${i.slice(0,6).toLowerCase()}`}const a=e.trim().match(/^rgba?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})(?:\s*,\s*(0|1|0?\.\d+))?\s*\)$/i);if(!a)return n;const o=a[4]===void 0?1:Number(a[4]),s=fe(t,n,n).slice(1).match(/../g).map(i=>parseInt(i,16));return`#${[1,2,3].map(i=>Math.round(Number(a[i])*o+s[i-1]*(1-o))).map(i=>i.toString(16).padStart(2,"0")).join("")}`}let we=null;var Lt;const Ut=((Lt=W("zcode"))==null?void 0:Lt.defaultPort)??9344;async function Bt(){try{if(Or(Ut)||!await zt("zcode"))return;nn(Ut),console.error("[main] usage pump ensured for zcode (passive start, 0.7.10)")}catch{}}P.app.whenReady().then(()=>{Bt(),setInterval(()=>void Bt(),1e4)});P.protocol.registerSchemesAsPrivileged([{scheme:"theme-asset",privileges:{standard:!0,secure:!0,supportFetchAPI:!0,stream:!0}}]);function mn(){we=new P.BrowserWindow({width:1200,height:800,webPreferences:{preload:u.join(__dirname,"preload.js"),contextIsolation:!0,nodeIntegration:!1}}),process.env.VITE_DEV_SERVER_URL?we.loadURL(process.env.VITE_DEV_SERVER_URL):we.loadFile(u.join(__dirname,"../renderer/dist/index.html"))}P.app.whenReady().then(()=>{P.protocol.handle("theme-asset",e=>{const n=decodeURIComponent(new URL(e.url).pathname.replace(/^\//,"")),t=ar(n);return t?new Response(f.readFileSync(t),{headers:{"Content-Type":Sa(t),"Cache-Control":"public, max-age=3600"}}):new Response("Theme asset not found",{status:404})}),mn()});function Sa(e){const n=u.extname(e).toLowerCase();return n===".jpg"||n===".jpeg"?"image/jpeg":n===".webp"?"image/webp":"image/png"}P.app.on("window-all-closed",()=>{process.platform!=="darwin"&&P.app.quit()});P.app.on("activate",()=>{P.BrowserWindow.getAllWindows().length===0&&mn()});const Dt=process.argv.find(e=>e.startsWith("--launch="));if(Dt){const[,e]=Dt.split("="),[n,t]=e.split(":");n&&t&&(console.log(`[main] Received launch args: ${n}:${t}`),setTimeout(async()=>{try{const r=await qt(n,t);r.success?(console.log(`[main] Launched ${n} with theme ${t} on port ${r.port}`),setTimeout(async()=>{try{console.log(`[main] Starting theme injection for ${n}:${t} on port ${r.port}`);const a=await on(n,t,r.port);console.log("[main] Injection result:",a)}catch(a){console.error("[main] Failed to inject theme:",a)}},3e3)):console.error(`[main] Failed to launch ${n}: ${r.error}`)}catch(r){console.error("[main] Launch error:",r)}},1e3))}P.ipcMain.handle("discover-apps",async()=>Un());P.ipcMain.handle("list-app-path-configurations",()=>Tn());P.ipcMain.handle("choose-custom-app-path",async(e,n)=>{if(process.platform!=="win32")return{success:!1,error:"自定义应用路径目前仅支持 Windows。"};const t=W(n);if(!t)return{success:!1,error:`Unknown app: ${n}`};const r={title:`选择 ${t.name} 的可执行文件`,buttonLabel:"选择此文件",properties:["openFile"],filters:[{name:`${t.name} 可执行文件`,extensions:["exe"]}]},a=we?await P.dialog.showOpenDialog(we,r):await P.dialog.showOpenDialog(r);if(a.canceled||a.filePaths.length===0)return{success:!1,cancelled:!0};try{return{success:!0,path:En(n,a.filePaths[0])}}catch(o){return{success:!1,error:(o==null?void 0:o.message)||String(o)}}});P.ipcMain.handle("clear-custom-app-path",(e,n)=>{try{return _n(n),{success:!0}}catch(t){return{success:!1,error:(t==null?void 0:t.message)||String(t)}}});P.ipcMain.handle("launch-app",async(e,n,t)=>qt(n,t));P.ipcMain.handle("apply-theme",async(e,n,t,r)=>on(n,t,r));P.ipcMain.handle("create-shortcut",async(e,n)=>{const t={...n,id:`${n.appId}-${n.themeId}-${Date.now()}`};return ca(t)});P.ipcMain.handle("list-themes",async(e,n)=>He(n).map(t=>({id:t.id,name:t.name,author:t.author,hero:or(t.id)})));P.ipcMain.handle("update-themes",async()=>ga());P.ipcMain.handle("get-status",async(e,n,t)=>{var a;return await zt(n)?{...await Nr(n,t||((a=W(n))==null?void 0:a.defaultPort)||9339),running:!0}:{installed:!1,menu:!1,targets:0,running:!1}});P.ipcMain.handle("remove-skin",async(e,n,t)=>Fr(n,t));P.ipcMain.handle("debug-targets",async(e,n)=>{try{const r=await(await fetch(`http://127.0.0.1:${n}/json/list`,{signal:AbortSignal.timeout(5e3)})).json();return{success:!0,count:r.length,raw:r,targets:r.map(a=>({id:a.id,type:a.type,url:a.url,title:a.title,webSocketDebuggerUrl:a.webSocketDebuggerUrl}))}}catch(t){return{success:!1,error:t.message}}});
